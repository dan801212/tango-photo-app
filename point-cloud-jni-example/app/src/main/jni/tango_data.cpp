  ...
  if (XYZ_ij->xyz_count <= TangoData::GetInstance().max_vertex_count) {
    if (TangoData::GetInstance().depth_buffer != nullptr &&
       ...
       for(int i=0; i<XYZ_ij->xyz_count; i++){
        ...
        if(XYZ_ij->xyz[i][0]>origin_maxX){
            origin_maxX = XYZ_ij->xyz[i][0];
        }
        if(XYZ_ij->xyz[i][1]<origin_minY){
            origin_minY = XYZ_ij->xyz[i][1];
        }
        if(XYZ_ij->xyz[i][1]>origin_maxY){
            origin_maxY = XYZ_ij->xyz[i][1];
        }
       }
...
//        LOGI("xyz => (%f,%f,%f), (%f,%f,%f), (%f,%f,%f), (%f,%f,%f),...,(%f, %f, %f)",
//        XYZ_ij->xyz[0][0], XYZ_ij->xyz[0][1], XYZ_ij->xyz[0][2],
//        XYZ_ij->xyz[1][0], XYZ_ij->xyz[1][1], XYZ_ij->xyz[1][2],
//        XYZ_ij->xyz[2][0], XYZ_ij->xyz[2][1], XYZ_ij->xyz[2][2],
//        XYZ_ij->xyz[3][0], XYZ_ij->xyz[3][1], XYZ_ij->xyz[3][2],
//        XYZ_ij->xyz[XYZ_ij->xyz_count][0], XYZ_ij->xyz[XYZ_ij->xyz_count][1], XYZ_ij->xyz[XYZ_ij->xyz_count][2]);
        }
  }
  TangoData::GetInstance().depth_buffer_size = new_size;
